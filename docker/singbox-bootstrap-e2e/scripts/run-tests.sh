...
for node in node-a node-b node-c; do
  for _ in $(seq 1 180); do
    if "${COMPOSE[@]}" exec -T "$node" bash -lc 'test -f /etc/sing-box/config.json && ss -lntu | grep -q ":11001"' >/dev/null 2>&1; then
      break
    fi
    sleep 1
  done
  "${COMPOSE[@]}" exec -T "$node" bash -lc 'test -f /etc/sing-box/config.json && ss -lntu | grep -q ":11001"'
done
...
set -euo pipefail
openssl verify -CAfile /etc/sing-box/node-link/ca.crt /etc/sing-box/node-link/node.crt >/dev/null
openssl verify -CAfile /etc/sing-box/node-link/ca.crt /etc/sing-box/node-link/client.crt >/dev/null
grep -q "\"client_authentication\": \"require-and-verify\"" /etc/sing-box/config.json
grep -q "\"client_certificate_path\": \"/etc/sing-box/node-link/client.crt\"" /etc/sing-box/config.json
if jq -e ".outbounds[]? | select((.tag // \"\") | startswith(\"exit-\")) | select(.tls.insecure == true)" /etc/sing-box/config.json >/dev/null; then
...
set -euo pipefail
sing-box check -c /state/client-$protocol.json >/dev/null
sing-box run -c /state/client-$protocol.json >/tmp/sing-box-client-$protocol.log 2>&1 &
pid=\$!
