...
- 管理节点：公网域名、入口开关、出口开关、节点间链路端口、证书状态、配置 hash。
...
生产不能沿用 POC 的自签证书和 `insecure: true`。

每个节点需要一个稳定域名：

```text
node-a.example.com -> node-a public IP
node-b.example.com -> node-b public IP
node-c.example.com -> node-c public IP
```

证书建议：

- 用户入口 TLS 协议使用公开可信证书。
- 节点间链路使用控制面内部 CA 签发的节点证书。
- 所有客户端配置里的 `server_name` 必须等于节点证书域名。
- 生产配置里的 `tls.insecure` 必须为 `false` 或不设置。

当前代码中 `SingBoxConfigBuilder._client_tls()` 仍写死：
...
生产建议使用 DNS 作为 `server_name`，不要依赖 IP 校验。
...
- 检查 node-link 证书 SAN 是否匹配 `--node-host`。
- 检查防火墙和云安全组提示项。
- 输出明确的下一步验证命令。
